...
import "./Blogs.css"; // Import the external CSS for styling
...
const Blog = () => {
  ...
  return (
    <div className="carousel-containerB">
      <h1>Blogs</h1>
      <Slider {...settings}>
        {blogs.map((blog, index) => (
          <div className="blog-card-containerB" key={index}>
            <div className="cardB">
              <img src={blog.image} alt={blog.title} className="card-imageB" />
              <div className="card-contentB">
                <h2 className="card-titleB">{blog.title}</h2>
                <p className="card-descriptionB">{blog.description}</p>
                <button className="card-btnB">{blog.buttonText}</button>
              </div>
            </div>
          </div>
        ))}
