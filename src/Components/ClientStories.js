...
import "./ClientStories.css"; // Import the external CSS for styling
...
const Blog = () => {
  ...
  return (
    <div className="carousel-containerC">
      <h1>ClientStories</h1>
      <Slider {...settings}>
        {blogs.map((blog, index) => (
          <div className="blog-card-containerC" key={index}>
            <div className="cardB">
              <img src={blog.image} alt={blog.title} className="card-imageC" />
              <div className="card-contentC">
                <h2 className="card-titleC">{blog.title}</h2>
                <p className="card-descriptionC">{blog.description}</p>
                <button className="card-btnC">{blog.buttonText}</button>
              </div>
            </div>
          </div>
        ))}
